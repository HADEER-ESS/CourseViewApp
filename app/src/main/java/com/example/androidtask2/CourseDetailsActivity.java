...
import com.example.androidtask2.Data.*;
...
public class CourseDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int contentDataIdentifier = getIntent().getIntExtra("content",0);
//        System.out.println("data " + data.get(contentDataIdentifier) );

        courseTitle = findViewById(R.id.course_Name_tv);
        courseTitle.setText(data.get(contentDataIdentifier).title);

        courseImage = findViewById(R.id.course_image_iv);
        courseImage.setImageResource(data.get(contentDataIdentifier).image);

        courseDescription = findViewById(R.id.course_description_tv);
        courseDescription.setText(data.get(contentDataIdentifier).description);
    }
}